...
def generate_ik_config(bodies, joints):
    ...
    # Calculate robot dimensions properly
    ...
    for body_name, body_info in bodies.items():
        ...
        if 'foot' in body_name.lower():
            foot_positions.append(pos[2])  # Z coordinate
        elif 'shoulder' in body_name.lower():
            shoulder_positions.append(pos[2])  # Z coordinate
    
    # Calculate robot height (from feet to shoulders)
    if foot_positions and shoulder_positions:
        robot_height = max(shoulder_positions) - min(foot_positions)
        print(f"Robot height (shoulders to feet): {robot_height:.3f}m")
    else:
    ...
    # Generate scale table with proper scaling
    base_scale = abs(scale_factor)  # Use absolute value
    for human_body in human_bodies:
        if 'pelvis' in human_body or 'spine' in human_body:
            config["human_scale_table"][human_body] = round(base_scale, 2)
            ...
            config["human_scale_table"][human_body] = round(base_scale * 0.9, 2)  # Arms slightly smaller
        else:
            config["human_scale_table"][human_body] = round(base_scale * 0.8, 2)  # Head/neck smaller
...
def main():
    ...
    try:
        ...
        # Save configuration
        output_file = "../../configs/dash/smplx_to_dash_from_urdf_fixed.json"
